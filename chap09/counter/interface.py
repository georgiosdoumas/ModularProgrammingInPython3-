def reset(ranges=None):
    ...
    _counts = {} # If _ranges is None, maps value to number of occurrences.
            # Otherwise, maps (min_value, max_value) to number of occurrences.
    ...
    if _ranges == None:
        ...
        for i in range(len(_ranges) - 1):
            if value >= _ranges[i] and value < _ranges[i+1]:
                ...
                break
        if key == None:
            raise RuntimeError("Value {} our of ranges.".format(value))
    try:
        _counts[key] += 1
    except KeyError:
        _counts[key] =1
...
##            num_occurencies = _counts.get((min_value, max_value), 0)
##            results.append((min_value, max_value, num_occurencies))
##        return results
        # The above lines are the code in the book, but we can do better with list comprehension
        return [ (_ranges[i], _ranges[i+1], _counts.get((_ranges[i], _ranges[i+1]), 0))  for i in range(len(_ranges) - 1) ] 
    else:
##        results = []
...
##        return results
        # The above lines are the code in the book, but we can do better with list comprehension
        return [(value, _counts[value]) for value in sorted(_counts.keys()) ]
